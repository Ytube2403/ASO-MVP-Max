...
from shared.paths import APPS_DIR, MASTER_KEYWORDS_DIR

def normalize_locale(filename):
    return extract_locale_from_filename(filename)

def scan_apps(apps_dir):
    # Any directory inside apps_dir that contains an "Input" folder is an app
    apps = []
...
def export_master_list(app_name, apps_dir):
    ...
    output_dir = MASTER_KEYWORDS_DIR
    os.makedirs(output_dir, exist_ok=True)

    output_file = os.path.join(output_dir, f"{app_name}_master.xlsx")
    wb.save(output_file)
    print(f"  [Success] Saved Master Keywords workbook to: {os.path.relpath(output_file, PROJECT_ROOT)}")
