...
Nếu đang cấu hình app mới từ `App_Template`, mở terminal/powershell tại thư mục `ASO-DEMO/App_Template` và chạy trực tiếp pipeline template:
...
Nếu chạy từ thư mục gốc `ASO-DEMO`, dùng đường dẫn script đầy đủ:

```powershell
python App_Template\run_pipeline.py --csv C:\duong-dan-den\MyNewApp_US_EN.csv --market US_EN
```
...
Sau khi chạy pipeline lọc từ khóa thành công, mở terminal tại thư mục gốc `ASO-DEMO` nếu muốn sử dụng thêm 2 công cụ bổ trợ:
...
```
Kết quả nằm trong thư mục `Master_Keywords/` dưới dạng Excel (mỗi sheet = 1 locale).
