...
Thư mục này chứa pipeline thực tế cho nhóm app Prank Sounds. Có thể chạy trực tiếp bằng `run_pipeline.py` hoặc chạy qua orchestrator `ASO-MVP/run_aso_filter.py` với file CSV có tên chứa `Prank`/`Pranky`.
...
Mở terminal/powershell tại thư mục `ASO-MVP/apps/Prank_Sounds` và chạy trực tiếp pipeline:
...
Hoặc chạy từ thư mục gốc `ASO-MVP` qua orchestrator để tự archive input vào `apps/Prank_Sounds/Input/[MMYYYY]/` và xuất output vào `apps/Prank_Sounds/Output/[MMYYYY]/`:
...
Pipeline hiện sử dụng các module chung trong `ASO-MVP/shared/`:
