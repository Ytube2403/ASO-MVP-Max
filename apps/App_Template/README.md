...
Nếu đang cấu hình app mới từ `App_Template`, mở terminal/powershell tại thư mục `ASO-MVP/apps/App_Template` và chạy trực tiếp pipeline template:
...
Nếu chạy từ thư mục gốc `ASO-MVP`, dùng đường dẫn script đầy đủ:
...
Sau khi chạy pipeline lọc từ khóa thành công, mở terminal tại thư mục gốc `ASO-MVP` nếu muốn sử dụng thêm 2 công cụ bổ trợ:
...
Template pipeline hien su dung cac module chung trong `ASO-MVP/shared/`:
