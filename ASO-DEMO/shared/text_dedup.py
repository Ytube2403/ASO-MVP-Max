...
def _indexed_review_pairs(indexes, keys, threshold):
    pairs = set()
    accent_groups = {}
    subset_groups = {}
    for index in indexes:
        item = keys[index]
        if item.accent_fold_review_key:
            accent_groups.setdefault(item.accent_fold_review_key, []).append(index)
        if item.tokenization_trusted:
            tokens = tuple(sorted(set(item.token_sequence_key)))
            for size in range(1, len(tokens) + 1):
                for subset in combinations(tokens, size):
                    subset_groups.setdefault((size, subset), []).append(index)
    ...
    for (size, _), group in subset_groups.items():
        anchors = [
            index
            for index in group
            if ceil(threshold * len(set(keys[index].token_sequence_key))) <= size
        ]
        for left_index in anchors:
            for right_index in group:
                if left_index < right_index:
                    pairs.add((left_index, right_index))
                elif right_index < left_index:
                    pairs.add((right_index, left_index))
    return sorted(pairs)
...
def deduplicate_candidates(records, sheet_name, language="", policy=None, tokenizer=None):
    ...
    if policy.get("enable_review_log", True):
        ...
        for left_index, right_index in _indexed_review_pairs(winner_indexes, keys, threshold):
